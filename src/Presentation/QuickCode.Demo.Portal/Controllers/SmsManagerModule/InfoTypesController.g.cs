...
namespace QuickCode.Demo.Portal.Controllers.SmsManagerModule
{
    ...
    public partial class InfoTypesController : BaseController
    {
        ...
        public async Task<IActionResult> List(InfoTypesData model)
        {
            ...
            model.List = mapper.Map<List<InfoTypesObj>>(listResponse.ToList());
            SetModelBinder(ref model);
            model.SelectedItem = new InfoTypesObj();
            return View("List", model);
        }
        ...
        {
            ModelBinder(ref model);
            var selected = mapper.Map<SmsManagerModuleContracts.InfoTypesDto>(model.SelectedItem);
        ...
        {
            ModelBinder(ref model);
            var request = mapper.Map<SmsManagerModuleContracts.InfoTypesDto>(model.SelectedItem);
        ...
        private async Task<InfoTypesData> FillModel(InfoTypesData model)
        {
            model.PageSize = pageSize;
            model.NumberOfRecord = (await pageClient.CountAsync());
            model.TotalPage = (model.NumberOfRecord / model.PageSize) + (model.NumberOfRecord % model.PageSize == 0 ? 0 : 1);
