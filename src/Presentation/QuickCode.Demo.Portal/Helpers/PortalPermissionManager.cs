    ...
    public class PortalPermissionManager : IPortalPermissionManager
    {
        ...
        private async Task<IEnumerable<SelectListItem>> GetSessionSelectListItem(string key)
        {
            ...
            if ((returnValue = GetHttpContextAccessor().HttpContext.Session.Get<IEnumerable<SelectListItem>>(key)) == null)
            {
                ...
                {
                    "PortalPermissionTypes" => (await portalPermissionTypesClient.PortalPermissionTypesListAsync())
                        .AsMultiSelectList("Id", "{0}", "Name"),
                    "PortalPermissions" => (await portalPermissionsClient.PortalPermissionsListAsync())
                        .AsMultiSelectList("Name", "{0}", "Name"),
                    _ => returnValue
                };
        ...
        private async Task<List<PortalPermissionGroupDto>> GetPortalPermissionGroup()
        {
            var key = "PortalPermissionGroups";
            List<PortalPermissionGroupDto> returnValue;
            if ((returnValue =
                    GetHttpContextAccessor().HttpContext!.Session.Get<List<PortalPermissionGroupDto>>(key)) !=
                null) 
                return returnValue;
            
            var portalPermissionGroups = await portalPermissionGroupsClient.PortalPermissionGroupsListAsync();
            
            returnValue =
                portalPermissionGroups.Where(i =>
                    i.IsActive &&
                    i.PermissionGroupName == SessionGroupName).ToList<PortalPermissionGroupDto>();
            GetHttpContextAccessor().HttpContext!.Session.Set(key, returnValue);
