...
public static class KafkaHelper
{
    internal static async Task<bool> GetTokenIsValid(IServiceProvider services, string token)
    {
        ...
        return !token.IsTokenExpired() && await authenticationsClient.ValidateAsync(token);
    }

    static async Task<List<KafkaEventsGetKafkaEventsResponseDto>> GetKafkaEvents(IServiceProvider services, IMemoryCache memoryCache)
    {
    ...
    {
        var configApiKey = configuration.GetValue<string>("QuickCodeApiKeys:UserManagerModuleApiKey");
        SetKafkaApiKeyToClients(kafkaEventsClient, configApiKey!);
        return (await kafkaEventsClient.GetKafkaEventsAsync()).ToList();
    }
        ...
        IMemoryCache memoryCache, HttpContext context)
    {
        ...
        kafkaEvent.TopicName = $"{kafkaEvent.TopicName}_{kafkaEvent.HttpMethod.ToLowerInvariant()}";
        return kafkaEvent;
    }
