...
namespace QuickCode.Demo.ApartmentManageModule.Persistence.Repositories
{
    ...
    {
        public async Task<RepoResponse<int>> MarkFlatInstallmentAsPaidAsync(int flatExpenseInstallmentsId, MarkFlatInstallmentAsPaidRequestDto updateRequest)
        {
            return await ExecuteWithExceptionHandling("FlatExpenseInstallment", async () =>
            {
                var sql = SqlLoader.Load(SqlScripts.FlatExpenseInstallment.Command.MarkFlatInstallmentAsPaid);
                var parameters = new
                {
