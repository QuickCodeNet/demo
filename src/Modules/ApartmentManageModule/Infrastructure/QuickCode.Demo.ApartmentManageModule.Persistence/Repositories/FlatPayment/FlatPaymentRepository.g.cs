...
namespace QuickCode.Demo.ApartmentManageModule.Persistence.Repositories
{
    public partial class FlatPaymentRepository : BaseRepository, IFlatPaymentRepository
    {
        ...
        public FlatPaymentRepository(ILogger<FlatPaymentRepository> logger, WriteDbContext writeContext, ReadDbContext readContext) : base(logger, "FlatPayment")
        {
            _writeContext = writeContext;
            _readContext = readContext;
        }

        public async Task<RepoResponse<FlatPaymentDto>> InsertAsync(FlatPaymentDto value)
        {
        ...
        public async Task<RepoResponse<bool>> UpdateAsync(FlatPaymentDto value)
        {
            return await ExecuteWithExceptionHandling("Update", async () =>
            {
                _writeContext.Set<Domain.Entities.FlatPayment>().Update(value.ToEntity());
                await _writeContext.SaveChangesAsync();
                return new RepoResponse<bool>(true, "Success");
            });
        }

        public async Task<RepoResponse<bool>> DeleteAsync(FlatPaymentDto value)
        {
            return await ExecuteWithExceptionHandling("Delete", async () =>
            {
                _writeContext.FlatPayment.Remove(value.ToEntity());
                await _writeContext.SaveChangesAsync();
                return new RepoResponse<bool>(true, "Success");
            });
        }

        public async Task<RepoResponse<FlatPaymentDto>> GetByPkAsync(int id)
        {
            return await ExecuteWithExceptionHandling("GetByPk", async () =>
            {
                var result =
                    from flat_payment in _readContext.FlatPayment
                    where flat_payment.Id.Equals(id)
                    select flat_payment;
                var response = await result.FirstOrDefaultAsync();
                return response == null ? new RepoResponse<FlatPaymentDto>
                {
                    Code = 404,
                    Message = "Not found in FlatPayment"
                }

                : new RepoResponse<FlatPaymentDto>(response.ToDto(), "Success");
            });
        }

        public async Task<RepoResponse<List<FlatPaymentDto>>> ListAsync(int? pageNumber = null, int? pageSize = null)
        {
            return await ExecuteWithExceptionHandling("Select", async () =>
            {
                ...
                {
                    return new RepoResponse<List<FlatPaymentDto>>
                    {
                        Code = 404,
                        Message = $"Page Number must be greater than {ConfigurationConstants.MinPageNumber}"};
                }

                var query = _readContext.FlatPayment.OrderBy(o => o.Id).AsQueryable();
                ...
                return new RepoResponse<List<FlatPaymentDto>>(result.ToDto(), "Success");
            });
        }
        ...
        public async Task<RepoResponse<int>> CountAsync()
        {
            ...
            {
                return new RepoResponse<int>(await _readContext.FlatPayment.CountAsync(), "Success");
            });
        }
    }
}