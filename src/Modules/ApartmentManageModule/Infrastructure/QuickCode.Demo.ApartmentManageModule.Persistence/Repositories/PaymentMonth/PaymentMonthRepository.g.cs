...
namespace QuickCode.Demo.ApartmentManageModule.Persistence.Repositories
{
    public partial class PaymentMonthRepository : BaseRepository, IPaymentMonthRepository
    {
        ...
        public PaymentMonthRepository(ILogger<PaymentMonthRepository> logger, WriteDbContext writeContext, ReadDbContext readContext) : base(logger, "PaymentMonth")
        {
            _writeContext = writeContext;
            _readContext = readContext;
        }

        public async Task<RepoResponse<PaymentMonthDto>> InsertAsync(PaymentMonthDto value)
        {
        ...
        public async Task<RepoResponse<bool>> UpdateAsync(PaymentMonthDto value)
        {
            return await ExecuteWithExceptionHandling("Update", async () =>
            {
                _writeContext.Set<Domain.Entities.PaymentMonth>().Update(value.ToEntity());
                await _writeContext.SaveChangesAsync();
                return new RepoResponse<bool>(true, "Success");
            });
        }

        public async Task<RepoResponse<bool>> DeleteAsync(PaymentMonthDto value)
        {
            return await ExecuteWithExceptionHandling("Delete", async () =>
            {
                _writeContext.PaymentMonth.Remove(value.ToEntity());
                await _writeContext.SaveChangesAsync();
                return new RepoResponse<bool>(true, "Success");
            });
        }

        public async Task<RepoResponse<PaymentMonthDto>> GetByPkAsync(int id)
        {
            return await ExecuteWithExceptionHandling("GetByPk", async () =>
            {
                var result =
                    from payment_month in _readContext.PaymentMonth
                    where payment_month.Id.Equals(id)
                    select payment_month;
                var response = await result.FirstOrDefaultAsync();
                return response == null ? new RepoResponse<PaymentMonthDto>
                {
                    Code = 404,
                    Message = "Not found in PaymentMonth"
                }

                : new RepoResponse<PaymentMonthDto>(response.ToDto(), "Success");
            });
        }

        public async Task<RepoResponse<List<PaymentMonthDto>>> ListAsync(int? pageNumber = null, int? pageSize = null)
        {
            return await ExecuteWithExceptionHandling("Select", async () =>
            {
                ...
                {
                    return new RepoResponse<List<PaymentMonthDto>>
                    {
                        Code = 404,
                        Message = $"Page Number must be greater than {ConfigurationConstants.MinPageNumber}"};
                }

                var query = _readContext.PaymentMonth.OrderBy(o => o.Id).AsQueryable();
                ...
                return new RepoResponse<List<PaymentMonthDto>>(result.ToDto(), "Success");
            });
        }
        ...
        public async Task<RepoResponse<int>> CountAsync()
        {
            ...
            {
                return new RepoResponse<int>(await _readContext.PaymentMonth.CountAsync(), "Success");
            });
        }
    }
}