...
namespace QuickCode.Demo.UserManagerModule.Api.Controllers
{
    public partial class PortalMenusController : QuickCodeBaseApiController
    {
        ...
        [HttpDelete("delete-portal-menu-items-with-module-name/{portalMenusKey}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(int))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> DeletePortalMenuItemsWithModuleNameAsync(string portalMenusKey)
        {
            var response = await mediator.Send(new DeletePortalMenuItemsWithModuleNameCommand(portalMenusKey));
            if (HandleResponseError(response, logger, "PortalMenu", $"PortalMenusKey: '{portalMenusKey}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }

        [HttpDelete("delete-portal-menu-items-with-model-name/{portalMenusKey}/{portalMenusName}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(int))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> DeletePortalMenuItemsWithModelNameAsync(string portalMenusKey, string portalMenusName)
        {
            var response = await mediator.Send(new DeletePortalMenuItemsWithModelNameCommand(portalMenusKey, portalMenusName));
            if (HandleResponseError(response, logger, "PortalMenu", $"PortalMenusKey: '{portalMenusKey}', PortalMenusName: '{portalMenusName}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
    }
