...
namespace QuickCode.Demo.UserManagerModule.Api.Controllers
{
    public partial class ApiMethodAccessGrantsController : QuickCodeBaseApiController
    {
        ...
        [HttpGet("get-api-method-access-grant-names/{apiMethodAccessGrantsPermissionGroupName}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(List<GetApiMethodAccessGrantNamesResponseDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetApiMethodAccessGrantNamesAsync(string apiMethodAccessGrantsPermissionGroupName)
        {
            var response = await mediator.Send(new GetApiMethodAccessGrantNamesQuery(apiMethodAccessGrantsPermissionGroupName));
            if (HandleResponseError(response, logger, "ApiMethodAccessGrant", $"ApiMethodAccessGrantsPermissionGroupName: '{apiMethodAccessGrantsPermissionGroupName}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
