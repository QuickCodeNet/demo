...
namespace QuickCode.Demo.UserManagerModule.Api.Controllers
{
    public partial class PortalPermissionTypesController : QuickCodeBaseApiController
    {
        ...
        public PortalPermissionTypesController(IMapper mapper, IMediator mediator, IServiceProvider serviceProvider, ILogger<PortalPermissionTypesController> logger)
        {
            this.mapper = mapper;
        ...
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(List<PortalPermissionTypesPortalPermissionGroups_RESTResponseDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> PortalPermissionTypesPortalPermissionGroups_RESTAsync(int portalPermissionTypesId)
        {
            var response = await mediator.Send(new PortalPermissionTypesPortalPermissionTypesPortalPermissionGroups_RESTQuery(portalPermissionTypesId));
            if (response.Code == 404)
            {
        ...
        [HttpGet("{portalPermissionTypesId}/portal-permission-groups/{portalPermissionGroupsId}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(PortalPermissionTypesPortalPermissionGroups_KEY_RESTResponseDto))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> PortalPermissionTypesPortalPermissionGroups_KEY_RESTAsync(int portalPermissionTypesId, int portalPermissionGroupsId)
        {
            var response = await mediator.Send(new PortalPermissionTypesPortalPermissionTypesPortalPermissionGroups_KEY_RESTQuery(portalPermissionTypesId, portalPermissionGroupsId));
            if (response.Code == 404)
            {
                var notFoundMessage = $"PortalPermissionTypesId: '{portalPermissionTypesId}', PortalPermissionGroupsId: '{portalPermissionGroupsId}' not found in PortalPermissionTypes Table";
                logger.LogWarning($"List Error: '{notFoundMessage}''");
                return NotFound(notFoundMessage);
            }
            else if (response.Code != 0)
