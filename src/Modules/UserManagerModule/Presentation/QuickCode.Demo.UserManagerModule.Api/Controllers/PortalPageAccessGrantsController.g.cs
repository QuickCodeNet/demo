...
using QuickCode.Demo.UserManagerModule.Application.Features.PortalPageAccessGrant;
...
namespace QuickCode.Demo.UserManagerModule.Api.Controllers
{
    public partial class PortalPageAccessGrantsController : QuickCodeBaseApiController
    {
        ...
        [HttpGet("get-portal-page-access-grants/{portalPageAccessGrantsPermissionGroupName}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(List<GetPortalPageAccessGrantsResponseDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetPortalPageAccessGrantsAsync(string portalPageAccessGrantsPermissionGroupName)
        {
            var response = await mediator.Send(new GetPortalPageAccessGrantsQuery(portalPageAccessGrantsPermissionGroupName));
            if (HandleResponseError(response, logger, "PortalPageAccessGrant", $"PortalPageAccessGrantsPermissionGroupName: '{portalPageAccessGrantsPermissionGroupName}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }

        [HttpGet("get-portal-page-access-grant/{portalPageAccessGrantsPermissionGroupName}/{portalPageAccessGrantsPortalPageDefinitionKey}/{portalPageAccessGrantsPageAction}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(List<GetPortalPageAccessGrantResponseDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetPortalPageAccessGrantAsync(string portalPageAccessGrantsPermissionGroupName, string portalPageAccessGrantsPortalPageDefinitionKey, PageActionType portalPageAccessGrantsPageAction)
        {
            var response = await mediator.Send(new GetPortalPageAccessGrantQuery(portalPageAccessGrantsPermissionGroupName, portalPageAccessGrantsPortalPageDefinitionKey, portalPageAccessGrantsPageAction));
            if (HandleResponseError(response, logger, "PortalPageAccessGrant", $"PortalPageAccessGrantsPermissionGroupName: '{portalPageAccessGrantsPermissionGroupName}', PortalPageAccessGrantsPortalPageDefinitionKey: '{portalPageAccessGrantsPortalPageDefinitionKey}', PortalPageAccessGrantsPageAction: '{portalPageAccessGrantsPageAction}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
