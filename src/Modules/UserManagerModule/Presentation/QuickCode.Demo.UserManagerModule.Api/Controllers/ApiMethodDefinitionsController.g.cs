...
namespace QuickCode.Demo.UserManagerModule.Api.Controllers
{
    public partial class ApiMethodDefinitionsController : QuickCodeBaseApiController
    {
        ...
        public ApiMethodDefinitionsController(IMapper mapper, IMediator mediator, IServiceProvider serviceProvider, ILogger<ApiMethodDefinitionsController> logger)
        {
            this.mapper = mapper;
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> ApiMethodDefinitionsKafkaEvents_KEY_RESTAsync(int apiMethodDefinitionsId, string kafkaEventsTopicName)
        {
            var response = await mediator.Send(new ApiMethodDefinitionsApiMethodDefinitionsKafkaEvents_KEY_RESTQuery(apiMethodDefinitionsId, kafkaEventsTopicName));
            if (response.Code == 404)
            {
