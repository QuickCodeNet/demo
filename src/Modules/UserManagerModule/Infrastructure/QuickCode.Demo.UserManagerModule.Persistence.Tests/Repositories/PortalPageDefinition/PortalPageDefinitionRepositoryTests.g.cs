...
namespace QuickCode.Demo.UserManagerModule.Persistence.Tests.Repositories
{
    public class PortalPageDefinitionRepositoryTests : IDisposable
    {
        ...
        private Mock<ILogger<PortalPageDefinitionRepository>> loggerMock = null;
        ...
        private PortalPageDefinitionRepository CreateRepository()
        {
            ...
            return new PortalPageDefinitionRepository(loggerMock.Object, writeContext, readContext);
        }
        ...
        public async Task InsertAsync_Should_Insert_Item_Successfully()
        {
            ...
            var itemDto = TestDataGenerator.CreateFake<PortalPageDefinitionDto>("tr");
            // Act
        ...
        public async Task InsertAsync_Should_Generate_Unique_Ids()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItem1 = TestDataGenerator.CreateFake<PortalPageDefinitionDto>();
            var fakeItem2 = TestDataGenerator.CreateFake<PortalPageDefinitionDto>("tr");
            // Act
        ...
        public async Task GetByPkAsync_Should_Return_Item_When_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var itemDto = TestDataGenerator.CreateFake<PortalPageDefinitionDto>();
        ...
        public async Task GetByPkAsync_Should_Return_NotFound_When_Item_Not_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var notExistsItem = TestDataGenerator.CreateFake<PortalPageDefinitionDto>("tr");
            // Act
            var result = await repository.GetByPkAsync(notExistsItem.Key);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Equal("Not found in PortalPageDefinition", result.Message);
        }
        ...
        [Fact]
        public async Task ListAsync_Should_Return_All_PortalPageDefinition()
        {
            // Arrange
            CreateDatabase();
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<PortalPageDefinitionDto>("tr");
        ...
        public async Task ListAsync_With_Pagination_Should_Return_Correct_Page()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<PortalPageDefinitionDto>("en", 5);
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
