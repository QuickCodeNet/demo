...
namespace QuickCode.Demo.UserManagerModule.Persistence.Tests.Repositories
{
    public class AspNetUserLoginRepositoryTests : IDisposable
    {
        ...
        private Mock<ILogger<AspNetUserLoginRepository>> loggerMock = null;
        ...
        private AspNetUserLoginRepository CreateRepository()
        {
            ...
            return new AspNetUserLoginRepository(loggerMock.Object, writeContext, readContext);
        }
        ...
        public async Task InsertAsync_Should_Insert_Item_Successfully()
        {
            ...
            var itemDto = TestDataGenerator.CreateFake<AspNetUserLoginDto>("tr");
            // Act
        ...
        public async Task InsertAsync_Should_Generate_Unique_Ids()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItem1 = TestDataGenerator.CreateFake<AspNetUserLoginDto>();
            var fakeItem2 = TestDataGenerator.CreateFake<AspNetUserLoginDto>("tr");
            // Act
        ...
        public async Task GetByPkAsync_Should_Return_Item_When_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var itemDto = TestDataGenerator.CreateFake<AspNetUserLoginDto>();
        ...
        public async Task GetByPkAsync_Should_Return_NotFound_When_Item_Not_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var notExistsItem = TestDataGenerator.CreateFake<AspNetUserLoginDto>("tr");
            // Act
            var result = await repository.GetByPkAsync(notExistsItem.LoginProvider, notExistsItem.ProviderKey);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Equal("Not found in AspNetUserLogin", result.Message);
        }
        ...
        [Fact]
        public async Task ListAsync_Should_Return_All_AspNetUserLogin()
        {
            // Arrange
            CreateDatabase();
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<AspNetUserLoginDto>("tr");
        ...
        public async Task ListAsync_With_Pagination_Should_Return_Correct_Page()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<AspNetUserLoginDto>("en", 5);
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
