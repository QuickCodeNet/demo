...
namespace QuickCode.Demo.UserManagerModule.Persistence.Tests.Repositories
{
    public class PortalPageAccessGrantRepositoryTests : IDisposable
    {
        ...
        private Mock<ILogger<PortalPageAccessGrantRepository>> loggerMock = null;
        ...
        private PortalPageAccessGrantRepository CreateRepository()
        {
            ...
            return new PortalPageAccessGrantRepository(loggerMock.Object, writeContext, readContext);
        }
        ...
        public async Task InsertAsync_Should_Insert_Item_Successfully()
        {
            ...
            var itemDto = TestDataGenerator.CreateFake<PortalPageAccessGrantDto>("tr");
            // Act
        ...
        public async Task InsertAsync_Should_Generate_Unique_Ids()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItem1 = TestDataGenerator.CreateFake<PortalPageAccessGrantDto>();
            var fakeItem2 = TestDataGenerator.CreateFake<PortalPageAccessGrantDto>("tr");
            // Act
        ...
        public async Task GetByPkAsync_Should_Return_Item_When_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var itemDto = TestDataGenerator.CreateFake<PortalPageAccessGrantDto>();
            var insertedItem = await repository.InsertAsync(itemDto);
            Assert.Equal(ResultCodeSuccess, insertedItem.Code);
            // Act
            var result = await repository.GetByPkAsync(insertedItem.Value.PermissionGroupName, insertedItem.Value.PortalPageDefinitionKey, insertedItem.Value.PageAction);
            // Assert
            Assert.Equal(ResultCodeSuccess, result.Code);
            itemDto.AssertPropertiesEqual(result.Value);
        }
        ...
        public async Task GetByPkAsync_Should_Return_NotFound_When_Item_Not_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var notExistsItem = TestDataGenerator.CreateFake<PortalPageAccessGrantDto>("tr");
            // Act
            var result = await repository.GetByPkAsync(notExistsItem.PermissionGroupName, notExistsItem.PortalPageDefinitionKey, notExistsItem.PageAction);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Equal("Not found in PortalPageAccessGrant", result.Message);
        }
        ...
        public async Task UpdateAsync_Should_Update_Item_Successfully()
        {
            ...
            var updates = insertedItem.GenerateRandomUpdates(["PermissionGroupName", "PortalPageDefinitionKey", "PageAction"]);
            ...
            var updatedItem = await repository.GetByPkAsync(insertedItem.PermissionGroupName, insertedItem.PortalPageDefinitionKey, insertedItem.PageAction);
        ...
        public async Task DeleteAsync_Should_Delete_Item_Successfully()
        {
            ...
            var deletedItem = await repository.GetByPkAsync(insertedItem.Value.PermissionGroupName, insertedItem.Value.PortalPageDefinitionKey, insertedItem.Value.PageAction);
            Assert.Equal(ResultCodeNotFound, deletedItem.Code);
        }

        [Fact]
        public async Task ListAsync_Should_Return_All_PortalPageAccessGrant()
        {
            // Arrange
            CreateDatabase();
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<PortalPageAccessGrantDto>("tr");
        ...
        public async Task ListAsync_With_Pagination_Should_Return_Correct_Page()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<PortalPageAccessGrantDto>("en", 5);
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
