        ...
        Dictionary<string, List<Dictionary<string, object>>> currentDbData)
    {
        ...
        foreach (var def in apiDefinitions)
        {
                ...
                modelsToDeleteSet.Contains((modelName, moduleName)))
            {
                try
                {
                    var kafkaEventsResponse = await _apiMethodDefinitionRepository.GetKafkaEventsForApiMethodDefinitionsAsync(key);
        ...
        foreach (var topicName in kafkaEventsToDelete)
        {
            try
            {
                var workflowsResponse = await _kafkaEventRepository.GetTopicWorkflowsForKafkaEventsAsync(topicName);
