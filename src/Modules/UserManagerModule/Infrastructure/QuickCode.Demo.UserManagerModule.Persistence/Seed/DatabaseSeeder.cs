...
public static class DatabaseSeeder
{
    ...
    private static async Task<Dictionary<string, List<Dictionary<string, object>>>> FetchCurrentDataFromDatabaseAsync(IServiceProvider serviceProvider, ILogger<WriteDbContext> logger)
    {
        ...
        try
        {
            var responsePortalPermissionGroups = await mediator.Send(new ListPortalPermissionGroupQuery(null, null));
            var responsePortalPermissions = await mediator.Send(new ListPortalPermissionQuery(null, null));
            var responsePortalMenus = await mediator.Send(new ListPortalMenuQuery(null, null));
            var responseApiMethodDefinitions = await mediator.Send(new ListApiMethodDefinitionQuery(null, null));
            var responseApiPermissionGroups = await mediator.Send(new ListApiPermissionGroupQuery(null, null));
            var responseTableComboboxSettings = await mediator.Send(new ListTableComboboxSettingQuery(null, null));
