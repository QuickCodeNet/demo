...
namespace QuickCode.Demo.OrderManagementModule.Api.Controllers
{
    public partial class ShipmentsController : QuickCodeBaseApiController
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetByOrderIdAsync(int shipmentOrderId, int? page, int? size)
        {
            if (page < 1)
            {
            ...
            var response = await service.GetByOrderIdAsync(shipmentOrderId, page, size);
        ...
        {
            var response = await service.GetByTrackingNumberAsync(shipmentTrackingNumber);
        ...
        {
            var response = await service.MarkAsShippedAsync(shipmentId, updateRequest);
        ...
        {
            var response = await service.MarkAsDeliveredAsync(shipmentId, updateRequest);
            if (HandleResponseError(response, logger, "Shipment", $"ShipmentId: '{shipmentId}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
