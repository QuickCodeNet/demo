...
{
    public class InsertCompanyCommandTests : IDisposable
    {
        ...
        public InsertCompanyCommandTests()
        {
        ...
        public async Task InsertAsync_Should_Return_Success_When_Valid_Request()
        {
            ...
            var fakeResponse = new RepoResponse<CompanyDto>(fakeDto, "Success");
            _repositoryMock.Setup(r => r.InsertAsync(It.IsAny<CompanyDto>())).ReturnsAsync(fakeResponse);
            // Act
            var result = await _service.InsertAsync(fakeDto);
            // Assert
            Assert.Equal(ResultCodeSuccess, result.Code);
            Assert.Equal(fakeDto, result.Value);
            _repositoryMock.Verify(r => r.InsertAsync(It.IsAny<CompanyDto>()), Times.Once);
        }
        ...
        public async Task InsertAsync_Should_Return_NotFound_When_Repository_Returns_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<CompanyDto>("tr");
            var fakeResponse = new RepoResponse<CompanyDto>
            {
