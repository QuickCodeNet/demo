...
namespace QuickCode.Demo.OnlineShopModule.Api.Controllers
{
    public partial class ProductReviewsController : QuickCodeBaseApiController
    {
        ...
        [HttpGet("get-recent-reviews/{productReviewsProductId:int}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(List<GetRecentReviewsResponseDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetRecentReviewsAsync(int productReviewsProductId)
        {
            var response = await service.GetRecentReviewsAsync(productReviewsProductId);
            if (HandleResponseError(response, logger, "ProductReview", $"ProductReviewsProductId: '{productReviewsProductId}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }

        [HttpGet("get-product-reviews/{productReviewsProductId:int}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(List<GetProductReviewsResponseDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetProductReviewsAsync(int productReviewsProductId)
        {
            var response = await service.GetProductReviewsAsync(productReviewsProductId);
            if (HandleResponseError(response, logger, "ProductReview", $"ProductReviewsProductId: '{productReviewsProductId}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
