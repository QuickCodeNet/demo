...
namespace QuickCode.Demo.OnlineShopModule.Api.Controllers
{
    public partial class CartItemsController : QuickCodeBaseApiController
    {
        ...
        [HttpGet("get-cart-items-with-total/{cartItemsCartId:int}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(List<GetCartItemsWithTotalResponseDto>))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> GetCartItemsWithTotalAsync(int cartItemsCartId)
        {
            var response = await service.GetCartItemsWithTotalAsync(cartItemsCartId);
            if (HandleResponseError(response, logger, "CartItem", $"CartItemsCartId: '{cartItemsCartId}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
    }
