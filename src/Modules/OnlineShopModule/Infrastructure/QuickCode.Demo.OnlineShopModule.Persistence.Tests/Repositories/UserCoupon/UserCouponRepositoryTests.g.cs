...
using Moq;
using QuickCode.Demo.OnlineShopModule.Persistence.Repositories;
...
namespace QuickCode.Demo.OnlineShopModule.Persistence.Tests.Repositories
{
    public class UserCouponRepositoryTests : IDisposable
    {
        ...
        private Mock<ILogger<UserCouponRepository>> loggerMock = null;
        private readonly ITestOutputHelper _output;
        public UserCouponRepositoryTests(ITestOutputHelper output)
        {
            _output = output;
            CreateDatabase();
        }

        private UserCouponRepository CreateRepository()
        {
            var writeContext = new WriteDbContext(writeOptions);
            var readContext = new ReadDbContext(readOptions);
            return new UserCouponRepository(loggerMock.Object, writeContext, readContext);
        }
        ...
        private void CreateDatabase()
        {
            ...
            loggerMock = new Mock<ILogger<UserCouponRepository>>();
        }
        ...
        public async Task InsertAsync_Should_Insert_Item_Successfully()
        {
            ...
            var itemDto = TestDataGenerator.CreateFake<UserCouponDto>("tr");
            // Act
        ...
        public async Task InsertAsync_Should_Generate_Unique_Ids()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItem1 = TestDataGenerator.CreateFake<UserCouponDto>();
            var fakeItem2 = TestDataGenerator.CreateFake<UserCouponDto>("tr");
            // Act
            ...
            Assert.NotEqual(result1.Value.UserId, result2.Value.UserId);
        }
        ...
        public async Task GetByPkAsync_Should_Return_Item_When_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var itemDto = TestDataGenerator.CreateFake<UserCouponDto>();
            var insertedItem = await repository.InsertAsync(itemDto);
            Assert.Equal(ResultCodeSuccess, insertedItem.Code);
            // Act
            var result = await repository.GetByPkAsync(insertedItem.Value.UserId, insertedItem.Value.CouponId);
            // Assert
            Assert.Equal(ResultCodeSuccess, result.Code);
            itemDto.AssertPropertiesEqual(result.Value);
        }
        ...
        public async Task GetByPkAsync_Should_Return_NotFound_When_Item_Not_Exists()
        {
            // Arrange
            var repository = CreateRepository();
            var notExistsItem = TestDataGenerator.CreateFake<UserCouponDto>("tr");
            // Act
            var result = await repository.GetByPkAsync(notExistsItem.UserId, notExistsItem.CouponId);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Equal("Not found in UserCoupon", result.Message);
        }
        ...
        public async Task UpdateAsync_Should_Update_Item_Successfully()
        {
            ...
            var updates = insertedItem.GenerateRandomUpdates(["UserId", "CouponId"]);
            ...
            var updatedItem = await repository.GetByPkAsync(insertedItem.UserId, insertedItem.CouponId);
        ...
        public async Task DeleteAsync_Should_Delete_Item_Successfully()
        {
            ...
            var deletedItem = await repository.GetByPkAsync(insertedItem.Value.UserId, insertedItem.Value.CouponId);
            Assert.Equal(ResultCodeNotFound, deletedItem.Code);
        }

        [Fact]
        public async Task ListAsync_Should_Return_All_UserCoupon()
        {
            // Arrange
            CreateDatabase();
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<UserCouponDto>("tr");
        ...
        public async Task ListAsync_With_Pagination_Should_Return_Correct_Page()
        {
            // Arrange
            var repository = CreateRepository();
            var fakeItems = TestDataGenerator.CreateFakes<UserCouponDto>("en", 5);
            foreach (var fakeItem in fakeItems)
            {
                var insertResponse = await repository.InsertAsync(fakeItem);
                if (insertResponse.Code != ResultCodeSuccess)
