...
namespace QuickCode.Demo.SmsManagerModule.Application.Features.MessageQueue
{
    public class GetByIdQuery : IRequest<Response<GetByIdResponseDto>>
    {
        ...
        public GetByIdQuery(int messageQueuesId)
        {
        ...
        public class GetByIdHandler : IRequestHandler<GetByIdQuery, Response<GetByIdResponseDto>>
        {
            ...
            public GetByIdHandler(ILogger<GetByIdHandler> logger, IMessageQueueRepository repository)
            {
                _logger = logger;
            ...
            {
                var returnValue = await _repository.GetByIdAsync(request.MessageQueuesId);
