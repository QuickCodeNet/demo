...
namespace QuickCode.Demo.SmsManagerModule.Application.Features.Message
{
    public class GetByIdQuery : IRequest<Response<GetByIdResponseDto>>
    {
        ...
        public GetByIdQuery(int messagesId)
        {
        ...
        public class GetByIdHandler : IRequestHandler<GetByIdQuery, Response<GetByIdResponseDto>>
        {
            ...
            public GetByIdHandler(ILogger<GetByIdHandler> logger, IMessageRepository repository)
            {
                _logger = logger;
            ...
            {
                var returnValue = await _repository.GetByIdAsync(request.MessagesId);
