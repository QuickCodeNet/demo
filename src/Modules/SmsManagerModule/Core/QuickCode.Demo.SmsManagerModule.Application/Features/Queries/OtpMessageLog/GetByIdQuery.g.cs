...
namespace QuickCode.Demo.SmsManagerModule.Application.Features.OtpMessageLog
{
    public class GetByIdQuery : IRequest<Response<GetByIdResponseDto>>
    {
        ...
        public GetByIdQuery(int otpMessageLogsId)
        {
        ...
        public class GetByIdHandler : IRequestHandler<GetByIdQuery, Response<GetByIdResponseDto>>
        {
            ...
            public GetByIdHandler(ILogger<GetByIdHandler> logger, IOtpMessageLogRepository repository)
            {
                _logger = logger;
            ...
            {
                var returnValue = await _repository.GetByIdAsync(request.OtpMessageLogsId);
