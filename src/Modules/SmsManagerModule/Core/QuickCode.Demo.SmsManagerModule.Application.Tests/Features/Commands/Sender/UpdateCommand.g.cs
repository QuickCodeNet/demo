...
namespace QuickCode.Demo.SmsManagerModule.Application.Tests.Features.Sender
{
    public class UpdateSenderCommandTests : IDisposable
    {
        ...
        private readonly Mock<ILogger<UpdateSenderCommand.UpdateSenderHandler>> _loggerMock;
        public UpdateSenderCommandTests()
        {
            _repositoryMock = new Mock<ISenderRepository>();
            _loggerMock = new Mock<ILogger<UpdateSenderCommand.UpdateSenderHandler>>();
        }
        ...
        public async Task Handle_Should_Return_Success_When_Valid_Command()
        {
            ...
            _repositoryMock.Setup(r => r.UpdateAsync(It.IsAny<SenderDto>())).ReturnsAsync(fakeResponse);
            _repositoryMock.Setup(r => r.GetByPkAsync(fakeDto.Id)).ReturnsAsync(new RepoResponse<SenderDto>());
            var handler = new UpdateSenderCommand.UpdateSenderHandler(_loggerMock.Object, _repositoryMock.Object);
            var command = new UpdateSenderCommand(fakeDto.Id, fakeDto);
            // Act
