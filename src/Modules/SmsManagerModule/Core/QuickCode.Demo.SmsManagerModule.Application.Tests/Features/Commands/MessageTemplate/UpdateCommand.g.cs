...
namespace QuickCode.Demo.SmsManagerModule.Application.Tests.Features.MessageTemplate
{
    public class UpdateMessageTemplateCommandTests : IDisposable
    {
        ...
        private readonly Mock<ILogger<UpdateMessageTemplateCommand.UpdateMessageTemplateHandler>> _loggerMock;
        public UpdateMessageTemplateCommandTests()
        {
            _repositoryMock = new Mock<IMessageTemplateRepository>();
            _loggerMock = new Mock<ILogger<UpdateMessageTemplateCommand.UpdateMessageTemplateHandler>>();
        }
        ...
        public async Task Handle_Should_Return_Success_When_Valid_Command()
        {
            ...
            _repositoryMock.Setup(r => r.UpdateAsync(It.IsAny<MessageTemplateDto>())).ReturnsAsync(fakeResponse);
            _repositoryMock.Setup(r => r.GetByPkAsync(fakeDto.Name)).ReturnsAsync(new RepoResponse<MessageTemplateDto>());
            var handler = new UpdateMessageTemplateCommand.UpdateMessageTemplateHandler(_loggerMock.Object, _repositoryMock.Object);
            var command = new UpdateMessageTemplateCommand(fakeDto.Name, fakeDto);
            // Act
