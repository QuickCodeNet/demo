...
{
    public class DeleteItemMessageQueueCommandTests : IDisposable
    {
        private const int ResultCodeSuccess = 0;
        private const int ResultCodeNotFound = 404;
        private readonly Mock<IMessageQueueRepository> _repositoryMock;
        private readonly Mock<ILogger<DeleteItemMessageQueueCommand.DeleteItemMessageQueueHandler>> _loggerMock;
        public DeleteItemMessageQueueCommandTests()
        {
            _repositoryMock = new Mock<IMessageQueueRepository>();
            _loggerMock = new Mock<ILogger<DeleteItemMessageQueueCommand.DeleteItemMessageQueueHandler>>();
        }
        ...
        public async Task Handle_Should_Return_Success_When_Valid_Command()
        {
            ...
            _repositoryMock.Setup(r => r.DeleteAsync(It.IsAny<MessageQueueDto>())).ReturnsAsync(fakeResponse);
            _repositoryMock.Setup(r => r.GetByPkAsync(fakeDto.Id)).ReturnsAsync(new RepoResponse<MessageQueueDto>());
            var handler = new DeleteItemMessageQueueCommand.DeleteItemMessageQueueHandler(_loggerMock.Object, _repositoryMock.Object);
            var command = new DeleteItemMessageQueueCommand(fakeDto.Id);
            // Act
