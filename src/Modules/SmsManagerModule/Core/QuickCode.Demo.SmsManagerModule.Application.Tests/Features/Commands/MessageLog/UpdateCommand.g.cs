...
namespace QuickCode.Demo.SmsManagerModule.Application.Tests.Features.MessageLog
{
    public class UpdateMessageLogCommandTests : IDisposable
    {
        ...
        private readonly Mock<ILogger<UpdateMessageLogCommand.UpdateMessageLogHandler>> _loggerMock;
        public UpdateMessageLogCommandTests()
        {
            _repositoryMock = new Mock<IMessageLogRepository>();
            _loggerMock = new Mock<ILogger<UpdateMessageLogCommand.UpdateMessageLogHandler>>();
        }
        ...
        public async Task Handle_Should_Return_Success_When_Valid_Command()
        {
            ...
            _repositoryMock.Setup(r => r.UpdateAsync(It.IsAny<MessageLogDto>())).ReturnsAsync(fakeResponse);
            _repositoryMock.Setup(r => r.GetByPkAsync(fakeDto.Id)).ReturnsAsync(new RepoResponse<MessageLogDto>());
            var handler = new UpdateMessageLogCommand.UpdateMessageLogHandler(_loggerMock.Object, _repositoryMock.Object);
            var command = new UpdateMessageLogCommand(fakeDto.Id, fakeDto);
            // Act
