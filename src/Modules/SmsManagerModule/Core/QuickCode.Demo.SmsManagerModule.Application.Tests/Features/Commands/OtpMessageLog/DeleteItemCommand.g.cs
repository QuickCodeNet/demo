...
{
    public class DeleteItemOtpMessageLogCommandTests : IDisposable
    {
        private const int ResultCodeSuccess = 0;
        private const int ResultCodeNotFound = 404;
        private readonly Mock<IOtpMessageLogRepository> _repositoryMock;
        private readonly Mock<ILogger<DeleteItemOtpMessageLogCommand.DeleteItemOtpMessageLogHandler>> _loggerMock;
        public DeleteItemOtpMessageLogCommandTests()
        {
            _repositoryMock = new Mock<IOtpMessageLogRepository>();
            _loggerMock = new Mock<ILogger<DeleteItemOtpMessageLogCommand.DeleteItemOtpMessageLogHandler>>();
        }
        ...
        public async Task Handle_Should_Return_Success_When_Valid_Command()
        {
            ...
            _repositoryMock.Setup(r => r.DeleteAsync(It.IsAny<OtpMessageLogDto>())).ReturnsAsync(fakeResponse);
            _repositoryMock.Setup(r => r.GetByPkAsync(fakeDto.Id)).ReturnsAsync(new RepoResponse<OtpMessageLogDto>());
            var handler = new DeleteItemOtpMessageLogCommand.DeleteItemOtpMessageLogHandler(_loggerMock.Object, _repositoryMock.Object);
            var command = new DeleteItemOtpMessageLogCommand(fakeDto.Id);
            // Act
