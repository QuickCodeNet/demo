...
namespace QuickCode.Demo.SmsManagerModule.Application.Tests.Features.OtpMessageQueue
{
    public class UpdateOtpMessageQueueCommandTests : IDisposable
    {
        ...
        private readonly Mock<ILogger<UpdateOtpMessageQueueCommand.UpdateOtpMessageQueueHandler>> _loggerMock;
        public UpdateOtpMessageQueueCommandTests()
        {
            _repositoryMock = new Mock<IOtpMessageQueueRepository>();
            _loggerMock = new Mock<ILogger<UpdateOtpMessageQueueCommand.UpdateOtpMessageQueueHandler>>();
        }
        ...
        public async Task Handle_Should_Return_Success_When_Valid_Command()
        {
            ...
            _repositoryMock.Setup(r => r.UpdateAsync(It.IsAny<OtpMessageQueueDto>())).ReturnsAsync(fakeResponse);
            _repositoryMock.Setup(r => r.GetByPkAsync(fakeDto.Id)).ReturnsAsync(new RepoResponse<OtpMessageQueueDto>());
            var handler = new UpdateOtpMessageQueueCommand.UpdateOtpMessageQueueHandler(_loggerMock.Object, _repositoryMock.Object);
            var command = new UpdateOtpMessageQueueCommand(fakeDto.Id, fakeDto);
            // Act
