using System;
using System.Threading;
using System.Threading.Tasks;
using Xunit;
using Moq;
using Microsoft.Extensions.Logging;
using QuickCode.Demo.SmsManagerModule.Application.Features.Campaign;
using QuickCode.Demo.SmsManagerModule.Application.Dtos.Campaign;
using QuickCode.Demo.SmsManagerModule.Application.Interfaces.Repositories;
using QuickCode.Demo.Common.Helpers;
using QuickCode.Demo.Common.Models;

namespace QuickCode.Demo.SmsManagerModule.Application.Tests.Features.Campaign
{
    public class InsertCampaignCommandTests : IDisposable
    {
        private const int ResultCodeSuccess = 0;
        private const int ResultCodeNotFound = 404;
        private readonly Mock<ICampaignRepository> _repositoryMock;
        private readonly Mock<ILogger<InsertCampaignCommand.InsertCampaignHandler>> _loggerMock;
        public InsertCampaignCommandTests()
        {
            _repositoryMock = new Mock<ICampaignRepository>();
            _loggerMock = new Mock<ILogger<InsertCampaignCommand.InsertCampaignHandler>>();
        }

        [Fact]
        public async Task Handle_Should_Return_Success_When_Valid_Command()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<CampaignDto>("tr");
            var fakeResponse = new RepoResponse<CampaignDto>(fakeDto, "Success");
            _repositoryMock.Setup(r => r.InsertAsync(It.IsAny<CampaignDto>())).ReturnsAsync(fakeResponse);
            var handler = new InsertCampaignCommand.InsertCampaignHandler(_loggerMock.Object, _repositoryMock.Object);
            var command = new InsertCampaignCommand(fakeDto);
            // Act
            var result = await handler.Handle(command, CancellationToken.None);
            // Assert
            Assert.Equal(ResultCodeSuccess, result.Code);
            Assert.Equal(fakeDto, result.Value);
            _repositoryMock.Verify(r => r.InsertAsync(It.IsAny<CampaignDto>()), Times.Once);
        }

        [Fact]
        public async Task Handle_Should_Return_NotFound_When_Repository_Returns_404()
        {
            // Arrange
            var fakeDto = TestDataGenerator.CreateFake<CampaignDto>("tr");
            var fakeResponse = new RepoResponse<CampaignDto>
            {
                Code = ResultCodeNotFound,
                Message = "Not found"
            };
            _repositoryMock.Setup(r => r.InsertAsync(It.IsAny<CampaignDto>())).ReturnsAsync(fakeResponse);
            var handler = new InsertCampaignCommand.InsertCampaignHandler(_loggerMock.Object, _repositoryMock.Object);
            var command = new InsertCampaignCommand(fakeDto);
            // Act
            var result = await handler.Handle(command, CancellationToken.None);
            // Assert
            Assert.Equal(ResultCodeNotFound, result.Code);
            Assert.Null(result.Value);
        }

        public void Dispose()
        {
        // Cleanup handled by xUnit
        }
    }
}