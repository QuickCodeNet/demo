...
namespace QuickCode.Demo.SmsManagerModule.Persistence.Repositories
{
    public partial class SmsSenderRepository : BaseRepository, ISmsSenderRepository
    {
        ...
        public SmsSenderRepository(ILogger<SmsSenderRepository> logger, WriteDbContext writeContext, ReadDbContext readContext) : base(logger, "SmsSender")
        {
            _writeContext = writeContext;
            _readContext = readContext;
        }

        public async Task<RepoResponse<SmsSenderDto>> InsertAsync(SmsSenderDto value)
        {
        ...
        public async Task<RepoResponse<bool>> UpdateAsync(SmsSenderDto value)
        {
            return await ExecuteWithExceptionHandling("Update", async () =>
            {
                _writeContext.Set<Domain.Entities.SmsSender>().Update(value.ToEntity());
                await _writeContext.SaveChangesAsync();
                return new RepoResponse<bool>(true, "Success");
            });
        }

        public async Task<RepoResponse<bool>> DeleteAsync(SmsSenderDto value)
        {
            return await ExecuteWithExceptionHandling("Delete", async () =>
            {
                _writeContext.SmsSender.Remove(value.ToEntity());
                await _writeContext.SaveChangesAsync();
                return new RepoResponse<bool>(true, "Success");
            });
        }

        public async Task<RepoResponse<SmsSenderDto>> GetByPkAsync(int id)
        {
            return await ExecuteWithExceptionHandling("GetByPk", async () =>
            {
                var result =
                    from sms_sender in _readContext.SmsSender
                    where sms_sender.Id.Equals(id)
                    select sms_sender;
                var response = await result.FirstOrDefaultAsync();
                return response == null ? new RepoResponse<SmsSenderDto>
                {
                    Code = 404,
                    Message = "Not found in SmsSender"
                }

                : new RepoResponse<SmsSenderDto>(response.ToDto(), "Success");
            });
        }

        public async Task<RepoResponse<List<SmsSenderDto>>> ListAsync(int? pageNumber = null, int? pageSize = null)
        {
            return await ExecuteWithExceptionHandling("Select", async () =>
            {
                ...
                {
                    return new RepoResponse<List<SmsSenderDto>>
                    {
                        Code = 404,
                        Message = $"Page Number must be greater than {ConfigurationConstants.MinPageNumber}"};
                }

                var query = _readContext.SmsSender.OrderBy(o => o.Id).AsQueryable();
                ...
                return new RepoResponse<List<SmsSenderDto>>(result.ToDto(), "Success");
            });
        }
        ...
        public async Task<RepoResponse<int>> CountAsync()
        {
            ...
            {
                return new RepoResponse<int>(await _readContext.SmsSender.CountAsync(), "Success");
            });
        }
    }
}