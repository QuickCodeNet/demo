...
namespace QuickCode.Demo.SmsManagerModule.Api.Controllers
{
    public partial class BlackListsController : QuickCodeBaseApiController
    {
        ...
        public BlackListsController(IMediator mediator, IServiceProvider serviceProvider, ILogger<BlackListsController> logger)
        {
        ...
        public async Task<IActionResult> ListAsync([FromQuery] int? page, int? size)
        {
            if (ValidatePagination(page, size) is {} error)
                return error;
            var response = await mediator.Send(new ListBlackListQuery(page, size));
        ...
        {
            var response = await mediator.Send(new TotalCountBlackListQuery());
        ...
        {
            var response = await mediator.Send(new GetItemBlackListQuery(id));
        ...
        {
            var response = await mediator.Send(new InsertBlackListCommand(model));
            if (HandleResponseError(response, logger, "BlackList") is {} responseError)
                return responseError;
        ...
        public async Task<IActionResult> UpdateAsync(int id, BlackListDto model)
        {
            ...
            var response = await mediator.Send(new UpdateBlackListCommand(id, model));
            if (HandleResponseError(response, logger, "BlackList", $"Id: '{id}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
        ...
        {
            var response = await mediator.Send(new DeleteItemBlackListCommand(id));
            if (HandleResponseError(response, logger, "BlackList", $"Id: '{id}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
        ...
        {
            var response = await mediator.Send(new GetByIdQuery(blackListsId));
        ...
        {
            var response = await mediator.Send(new GetByRecipientQuery(blackListsRecipient));
        ...
        {
            var response = await mediator.Send(new ExistsByRecipientQuery(blackListsRecipient));
            if (HandleResponseError(response, logger, "BlackList", $"BlackListsRecipient: '{blackListsRecipient}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
        ...
        {
            var response = await mediator.Send(new GetBlacklistCountQuery(blackListsReasonType));
        ...
        {
            var response = await mediator.Send(new AddToBlacklistCommand(blackListsRecipient, updateRequest));
            if (HandleResponseError(response, logger, "BlackList", $"BlackListsRecipient: '{blackListsRecipient}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }

        [HttpDelete("remove-from-sms-blacklist/{blackListsReasonType}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(int))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        public async Task<IActionResult> RemoveFromSmsBlacklistAsync(BlacklistReasonType blackListsReasonType)
        {
            var response = await mediator.Send(new RemoveFromSmsBlacklistCommand(blackListsReasonType));
            if (HandleResponseError(response, logger, "BlackList", $"BlackListsReasonType: '{blackListsReasonType}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
