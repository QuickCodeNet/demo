...
{
    public class PayoutLineItemRepositoryTests : IDisposable
    {
        private readonly Mock<IDbConnectionFactory> connectionFactoryMock;
        private readonly Mock<ILogger<PayoutLineItemRepository>> loggerMock;
        private readonly ITestOutputHelper _output;
        public PayoutLineItemRepositoryTests(ITestOutputHelper output)
        {
            _output = output;
            connectionFactoryMock = new Mock<IDbConnectionFactory>();
            loggerMock = new Mock<ILogger<PayoutLineItemRepository>>();
        }

        private PayoutLineItemRepository CreateRepository()
        {
            return new PayoutLineItemRepository(loggerMock.Object, connectionFactoryMock.Object);
        }
