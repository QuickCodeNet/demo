...
namespace QuickCode.Demo.EmailManagerModule.Api.Controllers
{
    ...
    {
        private readonly IOtpMessageQueueService service;
        private readonly ILogger<OtpMessageQueuesController> logger;
        private readonly IServiceProvider serviceProvider;
        public OtpMessageQueuesController(IOtpMessageQueueService service, IServiceProvider serviceProvider, ILogger<OtpMessageQueuesController> logger)
        {
        ...
        {
            var response = await service.GetByIdAsync(otpMessageQueuesId);
        ...
        {
            var response = await service.GetByOtpMessageAsync(otpMessageQueuesOtpMessageId);
        ...
        {
            var response = await service.GetPendingQueueAsync(otpMessageQueuesStatus);
        ...
        {
            var response = await service.UpdateStatusAsync(otpMessageQueuesId, updateRequest);
            if (HandleResponseError(response, logger, "OtpMessageQueue", $"OtpMessageQueuesId: '{otpMessageQueuesId}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
        ...
        {
            var response = await service.UpdatePriorityAsync(otpMessageQueuesId, updateRequest);
            if (HandleResponseError(response, logger, "OtpMessageQueue", $"OtpMessageQueuesId: '{otpMessageQueuesId}'") is {} responseError)
                return responseError;
            return Ok(response.Value);
        }
