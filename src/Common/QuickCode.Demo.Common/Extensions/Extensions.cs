    ...
    public static void AddMediatR<T>(this IServiceCollection services)
    {
        ...
        services.AddMediatR(cfg =>
        {
            ...
            var repositoryAssemblyName = assemblies.First(assembly =>
                    assembly.GetName().Name!.StartsWith(namespacePrefix) && assembly.GetName().Name!.EndsWith(".Persistence"))
                .GetReferencedAssemblies().First(i =>  i.Name!.StartsWith(namespacePrefix) && i.Name!.EndsWith(".Application"));
            var repositoryAssembly = Assembly.Load(repositoryAssemblyName);
            cfg.RegisterServicesFromAssembly(repositoryAssembly);
        });
    }
    ...
    public static void ParseJsonAsInitialData<T>(this MigrationBuilder migrationBuilder, List<string> fileList)
    {
        ...
        foreach (var file in fileList)
        {
            ...
            try
            {
                ...
                {
                    var entityType = domainEntityTypeAssembly.GetType($"{namespacePrefix}.Domain.Entities.{tableName.GetPascalCase()}");
                    if (entityType is null) continue;
                    
                    var columnNames = models[tableName][0].Keys.ToArray();
                    var objects = new object[models[tableName].Count, columnNames.Length];
                    var columnTypes = columnNames.Select(i => entityType.GetProperty(i.GetPascalCase())!.PropertyType.GetPropertyTypeName(migrationBuilder.ActiveProvider!));
                    ...
                    migrationBuilder.InsertData(tableName, columns: columnNames, columnTypes: columnTypes.ToArray(),
                        values: objects);
                }
            }
            catch (Exception ex)
